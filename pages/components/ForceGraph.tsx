...
let zoom = null;
const ForceGraph: React.FC<forceproperties> = ({zoomTo,initZoom,nodes,links,settings,chartwidth,chartheight,screenwidth,screenheight})  => {
  ...
  React.useEffect(() => {
    ...
    setTimeout(()=>{
        d3.select(chartRef.current).selectAll("[id^=linkpath_]")["_groups"][0].forEach((link:any,i:any)=>{
        ...
        let incr = 20;
        let nodecounter = 0;
        d3.select(chartRef.current).selectAll("[id^=node_]")["_groups"][0].forEach((node:any,i:any)=>{
  ...
  const showToolTip = (e:any,id:string,tid:string,type:number) =>{
      ...
      if (type === 1){
          nodes.forEach((node:any)=>{
             if (node.id === id){
                 Object.keys(node.sub).forEach((nkey:any)=>{
                     node.sub[nkey].forEach((tnode:any)=>{
                       if (tnode.id === node.id){
